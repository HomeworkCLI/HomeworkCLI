 ...
 * @Author: goEval
 * @LastEditors: goEval
 * @LastEditTime: 2020-07-19 19:16:52
 * @FilePath: \HomeworkCLI\HomeworkCLI.ts
 * @Github: https://github.com/heqyou_free
 */
// * HomeworkCLI Version 1.0.2
...
export class HomeworkCLI {
  ...
  clientLogin(username: string, password: string, isforce: boolean, usertype: number = 1):Promise<HomeworkResponse> {
    return new Promise<HomeworkResponse>((resolve, reject) => {
      this.post(this.baseUrl + 'jcservice/Login/clientLogin', this.encryptFormData({
        loginvalue: username,
        pwd: (() => {
   ...
   */
  listStuClassDoc(page: number): Promise<HomeworkResponse> {
    return this.post(this.baseUrl + 'jcservice/courseware/listStuClassDoc', this.encryptFormData({
      page: page,
      userid: this.userid,
    }));
  }
  ...
  getStudentDialogList(): Promise<HomeworkResponse> {
    return this.post(this.baseUrl + 'forum/FSNoticeHome-getStudentDialogList', this.encryptFormData({
  ...
  sendChatMessage(reuserid: string, type: number, content: string, totaltime: number = 0): Promise<HomeworkResponse> {
    return this.post(this.baseUrl + '/forum/FSNoticeHome-sendMessage', this.encryptFormData({
      totalTime: totaltime,
      type: type,
      userid: this.userid,
      content: content,
  ...
  getNoticeList(page: number): Promise<HomeworkResponse> {
    return this.post(this.baseUrl + 'forum/FSNoticeHome-getNoticeListByStudent', this.encryptFormData({
      studentid: this.userid,
      page: page,
    }));
  }
  ...
  getNewMessage(): Promise<HomeworkResponse> {
    return this.post(this.baseUrl + 'forum/FSNoticeHome-getUnreadMNcount', this.encryptFormData({
      userid: this.userid,
    }));
  }
  /**
  ...
  getUserRank(): Promise<HomeworkResponse> {
    return this.post(this.baseUrl + 'jcservice/User/getExpAndHonorInfo', this.encryptFormData({
      userid: this.userid,
    }));
  }
  /**
  ...
  getReadandCommentCount(noticeId: string): Promise<HomeworkResponse> {
    return this.post(this.baseUrl + 'forum/FSNoticeHome-getReadandCommentCount', this.encryptFormData({
      noticeId: noticeId,
    }));
  }
  /**
  ...
  checkModuleStatus(moduleid: number): Promise<HomeworkResponse> {
    return this.post(this.baseUrl + 'jcservice/SafeManage/checkStatus', this.encryptFormData({
      userid: this.userid,
      moduleid: moduleid,
    }));
  }
  ...
  todaySign(): Promise<HomeworkResponse> {
    return this.post(this.baseUrl + 'jcservice/Sign/addSign', this.encryptFormData({
      userid: this.userid,
    }));
  }
  /**
   ...
   * @return {Promise<HomeworkResponse>}
   */
  listStuLessonClass(page: number, pageSize: number = 1, bankname: string = '', keyword: string = ''): Promise<HomeworkResponse> {
  ...
  private encryptFormData(data: object): object {
    ...
    if (this.token === '') {
      ...
      return Object.assign(obj, {token: this.token});
    }
  }
